...
namespace GMAO
{
    public partial class da_reporter : Form
    {
        ...
        {
            if (e.CellElement.ColumnInfo is GridViewCommandColumn & e.CellElement.RowIndex != -1 & e.CellElement.ColumnIndex == 8)
            {
        ...
        private void action_tableau(object sender, GridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 8) // Retourner en cours
            {
                // Refactored data access
            }
            else if (e.ColumnIndex == 9) // Eyes
            {
        ...
        private void remplissage_tableau(int o)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = database.GetConnection(UserSession.ConnectionName))
                {
                    ...
                    {
                        cmdText = "select demande_achat.id, date_da, heure_da, createur, demandeur, delai, periode,da_reporter.reporter_par, da_reporter.nouvel_date from demande_achat inner join da_reporter on demande_achat.id = da_reporter.id_da where statut = @i";
                    }
                    else
                    {
                        cmdText = "select demande_achat.id, date_da, heure_da, createur, demandeur, delai, periode , periode,da_reporter.reporter_par, da_reporter.nouvel_date from demande_achat inner join da_reporter on demande_achat.id = da_reporter.id_da where statut = @i and demandeur = @v";
                    }
